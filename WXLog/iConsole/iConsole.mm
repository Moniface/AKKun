...
#define ActShtTagInfoAction 2300
#define ActShtTagLogLevel   2301
...
- (void)infoAction
{
	...
	UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@""
                                         ...
                                         destructiveButtonTitle:@"Clear Log"
                                              otherButtonTitles:@"Log Level", nil];

	sheet.actionSheetStyle = UIActionSheetStyleBlackOpaque;
    sheet.tag = ActShtTagInfoAction;
...
{
    if ( actionSheet.tag == ActShtTagInfoAction )
    {
        ...
        {   
            UIActionSheet* actSht = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                       cancelButtonTitle:@"Cancel"
                                                  destructiveButtonTitle:nil
                                                       otherButtonTitles:@"Disable", @"Error", @"Warm", @"Info", @"Debug", nil];
            
            actSht.tag = ActShtTagLogLevel;
            [actSht showInView:self.view];
        }
    }
    else if (actionSheet.tag == ActShtTagLogLevel)
    {
        if ( buttonIndex >=0 && buttonIndex <= 4 )
        {
            [WXLogUtils setLogOutFileLevel:buttonIndex];
        }
    }
}
...
//	NSLog(format, argList);
    ...
    if ([self sharedConsole].enabled)
    {
        ...
        try{
            ...
            NSAssert(0, @"iConsole日志写入异常，请检查异常原因。");
        }
    }
}
